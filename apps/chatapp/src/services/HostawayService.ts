...
class HostawayService {
    async getCurrentListing(reqData, authToken) {
        const response = await axios.get('https://api.hostaway.com/v1/listings', {
            ...
            .catch((err) => {
                console.error(`BotService -> getCurrentListing -> error: ${JSON.stringify(err)}`);
    ...
    async reservation(reqData, authToken, id=0) {
        try {
            ...
            if (error.response && error.response.status === 403) {
                ...
                console.error(`BotService -> reservation -> error: ${JSON.stringify(error)}`);
                return { error: true, msg: 'Internal server error' };
            }
        }
    }
    ...
    async cancelReservation(reqData, authToken) {
        ...
        .catch((err) => {
            console.error(`BotService -> cancelReservation -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async fetchConversations(reqData, authToken) {
        ...
        .catch((err) => {
            console.error(`BotService -> fetchConversations -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async fetchMessages(reqData, authToken) {
        ...
        .catch((err) => {
            console.error(`BotService -> fetchMessages -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async fetchReservations(reqData, authToken) {
        ...
        .catch((err) => {
            console.error(`BotService -> fetchReservations -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async getListings(authToken) {
        ...
        .catch((err) => {
            console.error(`BotService -> getListings -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }

...
class StayFlexiService {
    async getCurrentListing(reqData, authToken) {
        const response = await axios.get('https://api.hostaway.com/v1/listings', {
            ...
            .catch((err) => {
                LogService.error(`StayFlexiService -> getCurrentListing -> error: ${JSON.stringify(err)}`);
    ...
    async reservation(reqData, authToken) {
        ...
        .catch((err) => {
            LogService.error(`StayFlexiService -> reservation -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async cancelReservation(reqData, authToken) {
        ...
        .catch((err) => {
            LogService.error(`StayFlexiService -> cancelReservation -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async fetchConversations(hotelId, authToken) {
        const url = `https://api.stayflexi.com/core/api/v1/beservice/airbnb/get-chats?hotelId=${hotelId}`;
        ...
        try {
            ...
            LogService.error(`StayFlexiService -> fetchConversations -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' };
        }
    }

    async fetchMessages(reqData, authToken) {
        const url = `https://api.stayflexi.com/core/api/v1/beservice/airbnb/get-messages?hotelId=${reqData['hotelId']}&threadId=${reqData['threadId']}`;
        ...
        try {
            ...
            LogService.error(`StayFlexiService -> fetchMessages -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' };
        }
    }
    ...
    async fetchReservations(reqData, authToken) {
        ...
        .catch((err) => {
            LogService.error(`StayFlexiService -> fetchReservations -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async sendMessage(reqData, authToken) {
        // const url = `https://api.stayflexi.com/core/api/v1/beservice/airbnb/post-message?hotelId=${reqData['hotelId']}`;
        ...
        //             'X-SF-API-KEY': authToken
        //         },
        //         body: JSON.stringify({ threadId: reqData['threadId'], message: reqData['message'] })
        //     });
        ...
        const response = await API.post('/stayflexi/send-message', reqData)
            .then((res) => {
                return res.data
            })
            .catch((error) => {
                LogService.error(`StayFlexiService -> sendMessage -> error: ${JSON.stringify(error)}`);
                return { error: true, msg: 'Internal server error' }
            })
        return response;
    }
    ...
    async getListings(authToken, groupId) {
        ...
        .catch((err) => {
            LogService.error(`StayFlexiService -> getListings -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async getRoomsByHotel(authToken, hotelId) {
        const response = await fetch(`https://api.stayflexi.com/core/api/v1/beservice/hotelcontent?hotelId=${hotelId}`, {
            method: 'GET',
            headers: {
                "X-SF-API-KEY": `${authToken}`
            }
        ...
        .catch((err) => {
            LogService.error(`StayFlexiService -> getRoomsByHotel -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' };
        });
    ...
    async getPriceByPlanId(reqData, authToken) {
        ...
        .catch((err) => {
            LogService.error(`StayFlexiService -> getPriceByPlanId -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }

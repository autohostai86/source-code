...
class HostawayService {
    async getCurrentListing(reqData, authToken) {
        const response = await axios.get('https://api.hostaway.com/v1/listings', {
            ...
            .catch((err) => {
                LogService.error(`BotService -> getCurrentListing -> error: ${JSON.stringify(err)}`);
    ...
    async reservation(reqData, authToken, id=0) {
        ...
        // })
        // .catch((err) => {
        //     LogService.error(`BotService -> reservation -> error: ${JSON.stringify(err)}`);
        //     return { error: true, msg: 'Internal server error' }
        ...
        try {
            ...
            if (error.response && error.response.status === 403) {
                ...
                LogService.error(`BotService -> reservation -> error: ${JSON.stringify(error)}`);
                return { error: true, msg: 'Internal server error' };
            }
        }
    }
    ...
    async cancelReservation(reqData, authToken) {
        ...
        .catch((err) => {
            LogService.error(`BotService -> cancelReservation -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async fetchConversations(reqData, authToken) {
        const response = await axios.get(`https://api.hostaway.com/v1/conversations?limit=${reqData['limit']}&offset=${reqData['offset']}&includeResources=1`, {
            headers: {
                Authorization: `Bearer ${authToken}`
            }
        })
        ...
        .catch((err) => {
            LogService.error(`BotService -> fetchConversations -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async fetchMessages(reqData, authToken) {
        ...
        .catch((err) => {
            LogService.error(`BotService -> fetchMessages -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async fetchReservations(reqData, authToken) {
        ...
        .catch((err) => {
            LogService.error(`BotService -> fetchReservations -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async sendMessage(reqData, authToken) {
        ...
        .catch((err) => {
            LogService.error(`BotService -> sendMessage -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async getListings(authToken) {
        ...
        .catch((err) => {
            LogService.error(`BotService -> getListings -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
    ...
    async generateToken(reqData) {
        const response = await axios.post(`https://api.hostaway.com/v1/accessTokens`, reqData, {
            headers: {
    ...
    async getLast10Reservations(authToken) {
        const response = await axios.get(`https://api.hostaway.com/v1/reservations?limit=10&offset=0`, {
            headers: {
                // 'Content-Type': 'application/x-www-form-urlencoded'
                Authorization: `Bearer ${authToken}`
            }
        ...
        .catch((err) => {
            LogService.error(`BotService -> getLast10Reservations -> error: ${JSON.stringify(err)}`);
            return { error: true, msg: 'Internal server error' }
...
export default new HostawayService();

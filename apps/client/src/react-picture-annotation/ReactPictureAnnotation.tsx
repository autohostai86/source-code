...
export default class ReactPictureAnnotation extends React.Component<IReactPictureAnnotationProps> {
  ...
  public currentTransformer: ITransformer | any;

  private currentAnnotationData: IAnnotation[] = [];
  private selectedIdTrueValue: string | any;
  ...
  public componentDidUpdate = (preProps: IReactPictureAnnotationProps) => {
    ...
    // custome change
    if (this.props.zoomIn !== preProps.zoomIn) {
      this.scaleState.scale = 0.1 * this.props.zoomIn;
      this.scaleState.originX = 0;
      this.scaleState.originY = 0;

      // this.scaleState.originX = this.scaleState.originX;
      // this.scaleState.originY = this.scaleState.originY;
      this.onShapeChange();
      this.onImageChange();
    }
  };
    ...
    return (
      <>
        ...
        <div className="rp-stage">
          {/* <div>{JSON.stringify(this.shapes)}</div>; */}
          <canvas
            style={{ width, height }}
            className="rp-image"
  ...
  public onShapeChange = () => {
    if (this.canvas2D && this.canvasRef.current) {
      ...
      for (const item of this.shapes) {
        ...
        if (isSelected) {
          ...
          let checkConfigPresent = (this.state.elementsConfig as []).findIndex(
            (obj: any) => obj.id === item.getAnnotationData().id
          );

          if (
            (this.state.elementsConfig as []).findIndex(
              (obj: any) => obj.id === item.getAnnotationData().id
            ) === -1
          ) {
            this.state.elementsConfig.push({
              id: item.getAnnotationData().id,
              x,
              y,
              height,
              width,
            });
            this.setState({ elementsConfig: this.state.elementsConfig });
          } else {
            let updatedElementConfig = this.state.elementsConfig.map(
              (obj: any) => {
            ...
            this.setState({ elementsConfig: updatedElementConfig });
          }
        }
      }
    ...
    for (const annotateData of this.currentAnnotationData) {
      // this
      let index = (this.state.elementsConfig as []).findIndex(
        (obj: any) => obj.id === annotateData.id
      );
      if (this.state.elementsConfig[index]) {
        this.state.elementsConfig[index].comment = annotateData.comment;
  ...
  private setCanvasDPI = () => {
    ...
    if (currentCanvas && currentImageCanvas) {
      ...
      if (currentCanvas2D && currentImageCanvas2D) {
        ...
        currentCanvas2D.scale(2.3, 2.3);
        currentImageCanvas2D.scale(2.3, 2.3);
      }
    }
  };
  ...
  private onWheel = (event: React.WheelEvent<HTMLCanvasElement>) => {
    ...
    if (this.scaleState.scale < 0.07) {
      this.scaleState.scale = 0.07;
    }
